...
Task metadata uses one strict versionless schema, including optional `order`
for user-controlled task sequencing. Unknown fields remain errors; inverse
graph fields such as `blocks` and `children` are projections and are never
accepted as canonical metadata.
